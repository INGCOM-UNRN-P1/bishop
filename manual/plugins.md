---
title: "Guía de Extensión y Creación de Plugins: bishop"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de bishop"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-bishop-plugins)=
# Guía de Extensión y Plugins: bishop

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`bishop`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-bishop-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`bishop` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para bishop`.
- **Punto de Entrada Oficial**: `bishop.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-bishop-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para bishop."""
    name = "custom_rule"
...
Para que `bishop` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."bishop.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `bishop` la reconozca:
...
(manual-bishop-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `bishop` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = bishop.ejecutar_analisis(
...
(manual-bishop-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `bishop` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar bishop
        run: pip install -e ./bishop
        
      - name: Ejecutar Auditoría
        run: bishop check src/ include/ --json > reporte.json
````

---

(manual-bishop-plugins-ejercicios)=
